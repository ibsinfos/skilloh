...
{
	$DPID = intval(cleanit($_REQUEST['PID']));
	if($DPID > 0)
	{
		$sql="UPDATE posts SET active='3' WHERE PID='".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $DPID)."'";
		$conn->Execute($sql);
...
if($_POST['asub']=="1")
{
	...
	$sql="UPDATE posts SET active='".intval($aval2)."' WHERE PID='".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $APID)."'";
	$conn->Execute($sql);
	Stemplate::assign('message',$message);
}
//ACTIVE
