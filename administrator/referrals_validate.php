...
{
	$ARID = intval(cleanit($_REQUEST['RID']));
	if($ARID > 0)
	{
...
$queryselected = "select A.RID, B.username, C.username as refd from referrals A, members B, members C WHERE A.status='0' AND A.USERID=B.USERID AND A.REFERRED=C.USERID $addtosql $sort $sorthow limit $config[maximum_results]";
$query2 = "select A.*, B.username, C.username as refd from referrals A, members B, members C WHERE A.status='0' AND A.USERID=B.USERID AND A.REFERRED=C.USERID $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
...
if ($totalvideos > 0)
{
	...
	{	
		if($currentpage > 1) 
		{
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=1$add1' title='first page'>First</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=$theprevpage$add1'>Previous</a>&nbsp;";
		};
		$counter=0;
		...
		{
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=$lowercount$add1'>$lowercount</a>&nbsp;";
			$lowercount++;
			$counter++;
		}
		...
		{
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=$uppercounter$add1'>$uppercounter</a>&nbsp;";
			$uppercounter++;
		}
		if($currentpage < $toppage) 
		{
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=$thenextpage$add1'>Next</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/referrals_validate.php?page=$toppage$add1' title='last page'>Last</a>&nbsp;";
		};
	}
}
else
...
STemplate::display("administrator/referrals_validate.tpl");
