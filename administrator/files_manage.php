...
{
	$DFID = intval(cleanit($_REQUEST['FID']));
	if($DFID > 0)
	{
...
$queryselected = "select A.MSGFROM, B.*, C.username from inbox A, files B, members C WHERE A.FID>'0' AND A.FID=B.FID AND A.MSGFROM=C.USERID $addtosql $sort $sorthow limit $config[maximum_results]";
$query2 = "select A.MSGFROM, B.*, C.username from inbox A, files B, members C WHERE A.FID>'0' AND A.FID=B.FID AND A.MSGFROM=C.USERID $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
...
if ($totalvideos > 0)
{
	...
	{	
		if($currentpage > 1) 
		{
			$pagelinks.="<a href='$adminurl/files_manage.php?page=1$add1' title='first page'>First</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/files_manage.php?page=$theprevpage$add1'>Previous</a>&nbsp;";
		};
		$counter=0;
		...
		{
			$pagelinks.="<a href='$adminurl/files_manage.php?page=$lowercount$add1'>$lowercount</a>&nbsp;";
			$lowercount++;
			$counter++;
		}
		...
		{
			$pagelinks.="<a href='$adminurl/files_manage.php?page=$uppercounter$add1'>$uppercounter</a>&nbsp;";
			$uppercounter++;
		}
		if($currentpage < $toppage) 
		{
			$pagelinks.="<a href='$adminurl/files_manage.php?page=$thenextpage$add1'>Next</a>&nbsp;";
			$pagelinks.="<a href='$adminurl/files_manage.php?page=$toppage$add1' title='last page'>Last</a>&nbsp;";
		};
	}
}
else
{
	$error = "There are no files uploaded in conversations yet.";
}
...
STemplate::display("administrator/files_manage.tpl");
