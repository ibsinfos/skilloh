...
{	
	...
	$query="SELECT count(*) as total FROM orders A, posts B WHERE B.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND B.PID=A.PID $addc1 order by $addme2 $addme3";
	$results=$conn->execute($query);
	$counta = $results->fields['total'];
	STemplate::assign('counta',$counta);
	$query="SELECT count(*) as total FROM orders A, posts B WHERE B.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND B.PID=A.PID $addc2 order by $addme2 $addme3";
	$results=$conn->execute($query);
	$countb = $results->fields['total'];
	STemplate::assign('countb',$countb);
	$query="SELECT count(*) as total FROM orders A, posts B WHERE B.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND B.PID=A.PID $addc3 order by $addme2 $addme3";
	$results=$conn->execute($query);
	$countc = $results->fields['total'];
	STemplate::assign('countc',$countc);
	$query="SELECT count(*) as total FROM orders A, posts B WHERE B.USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND B.PID=A.PID $addc4 order by $addme2 $addme3";
	$results=$conn->execute($query);
