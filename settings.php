		...
		else
		{
				...
				if($config['enable_paypal'] == "1")
				{
					if($paypal != "")
					{
						...
						{
							$addme = ", pemail='".mysql_real_escape_string($paypal)."'";	
						}
					}
					else
					{
				...
				if($config['enable_alertpay'] == "1")
				{
					if($alertpay != "")
					{
						...
						{
							$addme .= ", aemail='".mysql_real_escape_string($alertpay)."'";	
						}
					}
					else
					{
				...
				{	
					...
					{
						$addmail = ",email='".mysql_real_escape_string($user_email)."', verified='0'";
						$_SESSION['VERIFIED']= 0;
						...
						$conn->execute($query);
						$query = "INSERT INTO members_verifycode SET USERID='".mysql_real_escape_string($SID)."', code='".mysql_real_escape_string($verifycode)."'";
						$conn->execute($query);
					...
					$query="UPDATE members SET fullname='".mysql_real_escape_string($fname)."' $addmail ,country='".mysql_real_escape_string($country)."' $addme , description='".mysql_real_escape_string($details)."', scriptolutionbankinfo='".mysql_real_escape_string($scriptolutionbankinfo)."', scriptolutionuserslogan='".mysql_real_escape_string($scriptolutionuserslogan)."' WHERE USERID='".mysql_real_escape_string($SID)."' AND status='1'";
