...
{	
	if($OID > 0)
	{
		...
		//
		$querycus = "select A.scriptolutioncustomorder from posts A, orders B where B.OID='".mysql_real_escape_string($OID)."' AND A.PID=B.PID limit 1"; 
		...
		{
			$query="SELECT A.*, B.gtitle, B.p1, B.price, B.price2, B.price3, B.USERID AS owner, B.days, B.ginst, C.username, E.username as buyer FROM orders A, posts B, members C, members E WHERE C.USERID=B.USERID AND E.USERID=A.USERID AND A.OID='".mysql_real_escape_string($OID)."' AND B.PID=A.PID limit 1";
		}
		else
		{
			$query="SELECT A.*, B.gtitle, B.p1, B.price, B.price2, B.price3, B.USERID AS owner, B.days, B.ginst, C.username, D.seo, E.username as buyer FROM orders A, posts B, members C, categories D, members E WHERE C.USERID=B.USERID AND B.category=D.CATID AND E.USERID=A.USERID AND A.OID='".mysql_real_escape_string($OID)."' AND B.PID=A.PID limit 1";
		}
		...
		elseif($buyer == $me)
		{
			...
			if($_POST['subrat'] == "1")
			{
				...
				$results=$conn->execute($query);
				$message = $lang['312'];
				$query="UPDATE orders SET status='5', cltime='".time()."' WHERE OID='".mysql_real_escape_string($OID)."' AND USERID='".mysql_real_escape_string($_SESSION['USERID'])."' AND status='4' limit 1";
				$results=$conn->execute($query);
		...
		$query="UPDATE fiverrscript_dotcom_notity SET scriptolution_unread='0' WHERE scriptolution_OID='".mysql_real_escape_string($OID)."' AND USERID='".mysql_real_escape_string($_SESSION['USERID'])."'";
		$results=$conn->execute($query);
