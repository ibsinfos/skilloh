			...
			<div class="left-side scriptolutionwidth842">
				<div class="whiteBox twoHalfs padding0 scriptolutionwidth800">  
                    ...
                    <div id="scriptolutionOrderingForm" class="scriptolutionpadding20"> 
                        ...
                        {if $funds gte $p.totalprice}<h2><a style="text-decoration:none" href="#" onclick="document.bal_form.submit();">{$lang412}</a></h2><br />{/if}
                        {if $afunds gte $p.totalprice}<h2><a style="text-decoration:none" href="#" onclick="document.scriptolution_mybal_form.submit();">{$lang518}</a></h2><br />{/if}
                        {if $scriptolution_local eq "1"}<h2><a style="text-decoration:none" href="{$baseurl}/bank_payment.php?item={$p.IID|stripslashes}">{$langl0}</a></h2><br />{/if}
                        {include file='order_scriptolution_stripe.tpl'}
                        <br />
                        <h2>{$lang489}: {if $scriptolution_cur_pos eq "1"}{$p.totalprice}{$lang197}{else}{$lang197}{$p.totalprice}{/if}</h2>
                    </div>
