 {section name=i loop=$posts} {if $smarty.section.i.iteration eq "6"}
<div style="padding:5px;">
    <center>
        {insert name=get_advertisement AID=3}
    </center>
</div>
{/if} {insert name=seo_clean_titles assign=title value=a title=$posts[i].gtitle}
<div class="box">
    <div class="c">
        <div class="holder">
            {if $posts[i].feat eq "1"}<span class="featured-label">featured</span>{/if}
            <div class="frame-img">
                {if $posts[i].youtube ne ""} {insert name=youtube_key value=a assign=ykey yt=$posts[i].youtube}
                <a href="{$baseurl}{$posts[i].seo|stripslashes}/{$posts[i].PID|stripslashes}/{$title}"><img alt="{$posts[i].gtitle|stripslashes}" src="http://i.ytimg.com/vi/{$ykey}/default.jpg" width="102" height="72" />
                    <br /><img alt="" style="position: absolute; left: 41px; top: 26px; border:none;" src="{$imageurl}/play.png" /></a>
                {else}
            ...
            <div class="frame">
                ...
                <p>{$posts[i].gdesc|stripslashes|mb_truncate:140:"...":'UTF-8'}<span>&nbsp;({$lang414} <a href="{$baseurl}{insert name=get_seo_profile value=a username=$posts[i].username|stripslashes}">{$posts[i].username|stripslashes|truncate:10:"...":true}</a>&nbsp;<span class="country {$posts[i].country}" title="{insert name=country_code_to_country value=a assign=userc code=$posts[i].country}{$userc}"></span>)</span>
                </p>
                <ul class="control">
                    <li>
                        <a href="{$baseurl}{$posts[i].seo|stripslashes}/{$posts[i].PID|stripslashes}/{$title}">{$lang105}</a>
                    </li>
                    {if $short_urls eq "1"} {insert name=get_short_url value=a assign=takento PID=$posts[i].PID seo=$posts[i].seo short=$posts[i].short title=$title}
                    <li style="width:30px;">
