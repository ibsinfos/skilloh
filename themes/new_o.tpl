                    ...
                    <div id="scriptolutionPostForm">
                        <form action="{$baseurl}new" enctype="multipart/form-data" id="new_gig" method="post">
                                    ...
                                    <div class="sparttwo">
                                        ...
                                        <div class="spartfour">
                                            ...
                                            <div style="padding-top:5px;">
                                                {if $scriptolution_cur_pos eq "1"}{$lang589} {$price}{$lang197}{else}{$lang63}{$price}{/if}
                                            </div>
                                            {elseif $price_mode eq "1"} {if $scriptolution_cur_pos eq "1"}{$lang589}
                                            <input class="textb" id="gprice" maxlength="3" name="gprice" size="3" type="text" value="{$gprice|stripslashes}" />{$lang197}{else}{$lang63}
                                            <input class="textb" id="gprice" maxlength="3" name="gprice" size="3" type="text" value="{$gprice|stripslashes}" />{/if} {elseif $price_mode eq "3"}{insert name=get_packs value=a assign=packs} {if $scriptolution_cur_pos eq "1"}
                                            <select name="gprice" id="gprice" class="textb">{section name=p loop=$packs}
                                                <option value="{$packs[p].ID|stripslashes}" {if $PACID eq $packs[p].ID|stripslashes}selected="selected" {/if}>{$packs[p].pprice|stripslashes}</option>{/section}</select>{$lang197}{else}{$lang197}
                                            <select name="gprice" id="gprice" class="textb">{section name=p loop=$packs}
                                                <option value="{$packs[p].ID|stripslashes}" {if $PACID eq $packs[p].ID|stripslashes}selected="selected" {/if}>{$packs[p].pprice|stripslashes}</option>{/section}</select>{/if} {/if}
                                        </div>
                                    </div>
                                </div>
                                {if $price_mode eq "0" OR $price_mode eq "2"}
                            ...
                            <div class="postgigbottom scriptolutionpadding20">
                                ...
                                <div class="form-entry">
                                    <div class="column-l">
                                        <label class="editgigformtitle new-badge" for="gig_order_requirement">{$lang73}</label>
                                        <br/>
                                ...
                                <div class="form-entry">
                                    ...
                                    <div class="column-r">
                                        <input class="text" id="scriptolutionjoblocation" name="scriptolutionjoblocation" type="text" value="{$scriptolutionjoblocation|stripslashes}" onFocus="geolocate()" /> {$lang618}:
                                        <input name="scriptolutioncity" class="textread scriptolutionauto" id="locality" disabled="true" readonly value="{$scriptolutioncity|stripslashes}"></input>
                                        {$lang619}:
                                        <input name="scriptolutionstate" class="textread scriptolutionauto" id="administrative_area_level_1" disabled="true" readonly value="{$scriptolutionstate|stripslashes}"></input>
                                        {$lang467}:
                                        <input name="scriptolutioncountry" class="textread scriptolutionauto" id="country" disabled="true" readonly value="{$scriptolutioncountry|stripslashes}"></input>
                                    </div>
                                </div>
                                <div class="scriptolutionclear"></div>
                                    ...
                                    <div id="additional-visuals" style="display:none">
                                        ...
                                        <div class="form-entry">
                                            <div id="youtube" class="row">
                                                <div class="column-l">
                                                    <label class="editgigformtitle" for="gig_youtube_url">{$lang89}</label>
                                                    <br/>({$lang90})
                                                </div>
                                                <div class="column-r">
                                ...
                                {include file='new_js.tpl'} {include file='new_multiples.tpl'} {include file='new_extras.tpl'} {include file='new_ship.tpl'}
                            </div>
