...
<form action="{$baseurl}send_track" class="nonajaxy" id="new_message" method="post">
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 content-text ">
        <div class="whitelinks">
            <div style="float:left">
                <a class="scriptolutiondmessageme achtung message_toggler" id="cancel-order-lin" href="#">{$lang325}</a>
                <a class="scriptolutionprobs achtung problems_toggler" id="cancel-order-link" href="#">{$lang326}</a>
            </div>
        </div>
        <div style="clear:both !important;"></div>
                    ...
                    <div id="problems_box" style="display:none;">
                        <div class="boxes">
                            ...
                            <div class="box rejection first reject_toggler">
                                <h3 class="got_prob_title"><input type="radio" class="radio_button" id="radio_reject" name="{$lang345}" value="{$lang345}" /> <span>{$lang344}</span></h3>
                                <p>{$lang346}</p>
                            </div>
                            {else}
                            <div class="box rejection first disabled">
                                <h3 class="got_prob_title"><input type="radio" class="radio_button" id="radio_reject" name="{$lang345}" value="{$lang345}" disabled="disabled" /> <span>{$lang344}</span></h3>
                                <p>{$lang346}</p>
                            </div>
                            {/if}
                            <div class="box mutual-cancel mid mutual_toggler mutual_toggler_buyer ">
                                <h3 class="got_prob_title"><input type="radio" class="radio_button" id="radio_mutual" name="{$lang330}" value="{$lang330}"    /> <span>{$lang347}</span></h3>
                                <p>{$lang348}</p>
                            </div>
                            <div class="box notice last support_toggler">
                                <h3 class="got_prob_title"><input type="radio" class="radio_button" id="radio_support" name="{$lang334}" value="{$lang334}"/> <span>{$lang335}</span></h3>
                                <p>{$lang336}</p>
                            </div>
                        </div>
                    </div>
