        			...
        			<div id="customerGrid">
                        ...
                        <div class="grid">
							<div class="hor-scroll">
	    	    	        	...
	    	    	        	<thead>
	            	                <tr class="headings">
                                        <th ><span class="nobr"><a href="gigs_manage.php?page={$currentpage}&sortby=PID&sorthow={if $sortby eq "PID"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}" name="id" class="{if $sortby eq "PID"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">ID</span></a></span></th>
                                        <th ><span class="nobr"><a href="gigs_manage.php?page={$currentpage}&sortby=gtitle&sorthow={if $sortby eq "gtitle"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}" name="gtitle" class="{if $sortby eq "gtitle"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">Title</span></a></span></th>
                                        <th ><span class="nobr"><a href="gigs_manage.php?page={$currentpage}&sortby=username&sorthow={if $sortby eq "username"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}" name="username" class="{if $sortby eq "username"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">Poster</span></a></span></th>
                                        <th ><span class="nobr"><a href="gigs_manage.php?page={$currentpage}&sortby=time_added&sorthow={if $sortby eq "time_added"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}" name="time_added" class="{if $sortby eq "time_added"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">Date Created</span></a></span></th>
                                        <th ><span class="nobr"><a href="gigs_manage.php?page={$currentpage}&sortby=active&sorthow={if $sortby eq "active"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}" name="active" class="{if $sortby eq "active"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">Active</span></a></span></th>
	    	    	    		...
	    	    	    		<tbody>
                                    ...
                                    <tr id="" >
                                        ...
                                        <td class=" ">{$results[i].gtitle|stripslashes|truncate:300:"...":true}{include file="administrator/gigs_instant_bit.tpl" subresults=$results[i]}{include file="administrator/gigs_extras_bit.tpl" subresults=$results[i]}</td>
                                        <td class=" ">{$results[i].username|stripslashes|truncate:20:"...":true}</td>
                                        <td class=" ">{$results[i].time_added|date_format:"%b %e, %Y"}</td>
                                        <td class=" ">{if $results[i].active eq "1"}Active{elseif $results[i].active eq "0"}Not Active{else}Suspended{/if}</td>
                                        <td class=" last"><a href="gigs_edit.php?PID={$results[i].PID}">Edit</a>&nbsp;|&nbsp;<a href="gigs_manage.php?page={$currentpage}&sortby={$sortby}&sorthow={$sorthow}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&title={$title}&gtitle={$gtitle}&username={$username}&active={$active}{/if}&delete=1&PID={$results[i].PID}">Delete</a></td>
                                	</tr>
                                    {/section}
                                    <tr>
                                    	<td colspan="6">
