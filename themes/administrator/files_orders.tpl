            ...
            <div id="page:main-container">
				<div class="columns ">
        			...
        			<div id="customerGrid">
        				...
        				<tr>
                    		<td class="pager">
                            	Showing {if $total gt 0}{$beginning} - {$ending} of {/if}{$total} Files
                    		</td>
                        ...
                        <div class="grid">
							<div class="hor-scroll">
	    	    	        	...
	    	    	        	<thead>
	            	                <tr class="headings">
                                        <th ><span class="nobr"><a href="files_orders.php?page={$currentpage}&sortby=FID&sorthow={if $sortby eq "FID"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&details={$details}&active={$active}{/if}" name="id" class="{if $sortby eq "FID"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">ID</span></a></span></th>
                                        <th ><span class="nobr"><a href="files_orders.php?page={$currentpage}&sortby=details&sorthow={if $sortby eq "details"}{if $sorthow eq "desc"}asc{else}desc{/if}{else}{$sorthow}{/if}{if $search eq "1"}&fromid={$fromid}&$toid={$toid}&details={$details}&active={$active}{/if}" name="details" class="{if $sortby eq "details"}sort-arrow-{if $sorthow eq "desc"}desc{else}asc{/if}{else}not-sort{/if}"><span class="sort-title">File Name</span></a></span></th>
	            	            	...
	            	            	<tr class="filter">
                                        ...
                                        <th ><input type="text" name="fname" id="fname" value="{$fname|stripslashes}" class="input-text no-changes"/></th>
	    	    	    		...
	    	    	    		<tbody>
                                    ...
                                    <tr id="" >
                                        <td align="center">{$results[i].FID}</td>
                                        <td class=" ">
                                        	{if $results[i].FID GT "0"}
                                            {insert name=file_details value=a assign=fd fid=$results[i].FID}
                                        ...
                                        <td class=" ">
                                        	{$results[i].username|stripslashes}
                                        </td>
                                        <td class=" last">
                                            <a href="files_orders.php?page={$currentpage}&sortby={$sortby}&sorthow={$sorthow}{if $search eq "1"}&fromid={$fromid}&toid={$toid}&details={$details}&active={$active}{/if}&delete=1&FID={$results[i].FID}">Delete</a>
                                        </td>
                                	</tr>
                                    {/section}
                                    <tr>
