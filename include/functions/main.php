...
{
    global $config,$conn;
	$query = "SELECT A.*, C.username, C.country, C.toprated from scriptolutionrequests A, members C where A.active='1' AND A.USERID=C.USERID order by REQUESTID desc limit 5"; 
	$results = $conn->execute($query);
	$returnthis = $results->getrows();
	return $returnthis;
}
